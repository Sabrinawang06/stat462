...
| From the VIF table we can see density has VIF of 25.061323 which mean severe collinearity (the plot shows the linear relationship between density and alcoho). So we have to remove this predictor, and this is the new VIF result.
...
In conclusion, we should use the model with 7 predictors bacause it has relatively big $R^2_{adj}$, low $C_p$, low aic and low bic. The final models includes volatile.acidity, residual.sugar, chlorides, free.sulfur.dioxide, sulphates, alcohol and dummy variables.

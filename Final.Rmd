...
```
  ...
  There are the points with leverage greater then the threshhold 3*p/n:
```{r, echo=FALSE}
which(h>thresh3)
```
...
\setcounter{table}{4}
```{r echo=FALSE, comment=FALSE, results='asis',message=FALSE}
require(stargazer)
#stargazer(linear_red)
stargazer(best_ordinal,header=FALSE)
par(mfrow=c(2,2))
...
```
  ...
  Accoding to these plot, there are a lot of observations that have very high leverage (red points in leverage plot are observations with leverage greater than threshold 2p/n) which is most likely due to the fact that a logistic model would better represent them
